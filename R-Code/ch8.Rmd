...
plot(prune.carseats);text(prune.carseats,pretty=1)
```
...
#My working, with CV Error
cv.err=double(100)
test.err=double(100)

boost.boston=gbm(medv~.,data=Boston[train,],distribution="gaussian",n.trees=10000,shrinkage=0.01,interaction.depth=4, cv.folds = 10)
predmat=predict(boost.boston,newdata=Boston[-train,],n.trees = n.trees)
test.err <- with(Boston[-train,],apply( (predmat-medv)^2,2,mean))
cv.err <- boost.boston$cv.error[n.trees]

matplot(n.trees,cbind(cv.err, test.err),pch=19,ylab="Mean Squared Error", col=c("red","blue"), xlab="# Trees",main="Boosting Test Error")
```
